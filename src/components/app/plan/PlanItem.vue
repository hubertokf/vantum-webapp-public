<template>
  <div class="plan-item" @click="open()" :key="object.id">
    ...
    <div class="tools" v-else>
      <div class="deleteItem" v-on:click.stop="removePlanFromProject()" v-if="!example">
  ...
  methods: {
    ...
    open() {
      ...
      this.$parent.$router.push({
        name: "Plan",
        params: { planObject: this.object, planId: objectID, share: this.share, example: this.example },
      });
    ...
    deleteItem() {
      if (!this.share) {
        if (this.object.project) {
          this.removePlanFromProject().then(() => {
            this.$store.dispatch("deletePlan", this.object.id).then(() => {
              this.$notify({
                group: "global",
                type: "is-success",
                duration: 2000,
    ...
    removePlanFromProject() {
      return this.$store.dispatch("removePlanFromProject", this.object);
    }
  }
};
</script>
