          ...
          <div class="signup-box">
            ...
            >
              <b-step-item type="is-success" label="Usuário" :clickable="isStepsClickable">
                ...
                <signup-form :showButton="false" :redirect="false" :userObj="user" ref="signupForm"></signup-form>
              ...
              <b-step-item type="is-success" label="Empresa" :clickable="isStepsClickable">
                <h1>Cadastro de Empresa</h1>
                <company-form :user="user" :showButton="false" ref="companyForm"></company-form>
              ...
              <b-step-item type="is-success" label="Finalizar" :clickable="isStepsClickable" class="finish-step">
                <div class="thanks">Obrigado!</div>
                <div class="finish-message">Entraremos em contato para validar seus dados e habilitar a sua conta empresarial.</div>
...
import CompanyForm from "@/components/app/company/CompanyForm";
...
export default {
  ...
  mounted() {
    if (this.inputUser) {
      ...
      if (!this.user.phone) {
        ...
        this.activeStep = 2
      }else if(!this.user.professional) {
        this.activeStep = 3
      }else{
        this.activeStep = 4
      }
  ...
  methods: {
    saveAccount (){
      this.$refs.signupForm.submit().then((user)=>{
        this.user = user
        this.$refs.phoneForm.setPhone(user.phone)
        this.activeStep++
      })
    },
    savePhone (){
    ...
    saveCompany (){
      this.$refs.companyForm.submit().then((company)=>{
        this.company = company
        page('/signup/company/success');
